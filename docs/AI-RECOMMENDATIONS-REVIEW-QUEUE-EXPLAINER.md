...
- **Trigger Analysis** (kanan) — admin bisa **memicu** agent menganalisa satu channel manual (tak cuma menunggu kegagalan). Ada elapsed timer + tombol Batalkan + timeout 120s (agent LLM bisa lambat / 429).
  - **Scope kategori (opsional)** — picker Product Type di samping channel. Jika dipilih, trigger mengirim **`productTypeId`** (ObjectId) + **sample produk representatif** dari type tsb; **backend menurunkan kategori** dari `ProductType.categorySlug` (paritas Phase 0B dengan `/generate-jolt` — satu sumber kebenaran, sama dengan yang diresolusi saat publish). Jadi JOLT digenerate untuk **channel × kategori yang tepat**, bukan `default` dengan sample kosong. Dikosongkan = perilaku lama (kategori `default`).
