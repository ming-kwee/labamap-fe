...
| [`06-import-channel-native.md`](06-import-channel-native.md) | **Use case B — IMPORT.** Produk yang hanya ada di channel → **master BARU** (draft). Dua use case (A reconcile vs B import); alur import; kontrak `/import/preview`+`/import`; mapper + dedup; batas (browse/list belum) |
| [`07-import-image-rehosting.md`](07-import-image-rehosting.md) | **PLAN (belum diimplementasi).** Re-host gambar import: URL asli channel → storage platform (GCS/CDN) supaya re-push tak kehilangan gambar; pakai ulang modul `media/`; timing (inline/async/lazy); dedup; investigasi `product.images=[]` |
| [`08-shopee-reverse-sync-plan.md`](08-shopee-reverse-sync-plan.md) | **PLAN (belum dieksekusi).** Onboarding **Shopee** ke engine reverse: apa yang sudah ada (transform terverifikasi) vs kurang (pull ber-signature, multi-fetch, webhook-as-trigger, images, import); kenapa Shopee≠Shopify; desain data-driven per-gap (G1–G6); fase S0–S5; blocker sample push |
| [`09-tiktokshop-reverse-sync-plan.md`](09-tiktokshop-reverse-sync-plan.md) | **PLAN (belum dieksekusi).** Onboarding **TikTok Shop**: `reverseSyncConfig` **belum ada sama sekali**; kenapa TikTok≠Shopee≠Shopify (read header+signed, list POST search, `sales_attributes` name-based inline, gambar SKU inline); **4 celah framework generik** (T-G1 header pada SIGNED, T-G2 secret dari store cred, T-G3 POST list, T-G4 axis `INLINE_NAME_VALUE`) + seed; fase T-S0..S4; blocker sample GET/search/webhook |
| [`shopify/01-category-attributes-reverse-shape-b.md`](shopify/01-category-attributes-reverse-shape-b.md) | **RANCANGAN (belum diimplementasi).** Reverse **SHAPE B** — category attribute reference-object (Shopify metafield) balik ke `channelData`. Kenapa SHAPE A trivial tapi SHAPE B butuh 2 inversi (`handle→nama`, `Metaobject GID→TaxonomyValue GID`); op baru generic **`METAFIELD_INVERSE`** (mirror `BUILD_METAFIELD_LIST`); reuse artefak forward Tahap A–D dibalik; payoff round-trip dirty-detection; fase R-B0..B5 |
