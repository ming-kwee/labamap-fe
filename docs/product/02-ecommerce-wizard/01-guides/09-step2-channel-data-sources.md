...
The backend will cache the full category tree per channel/store in a `channel_category_cache` collection (TTL 24h, indexed by `(channelType, storeId, parentId)`), warmed nightly by a `CategorySyncJob`. `ChannelStepSchemaService` will pre-populate `categoryTreeConfig.selectedPath` from the saved `categoryId` to avoid a round-trip on form load.
